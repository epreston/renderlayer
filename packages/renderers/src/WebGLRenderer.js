...
class WebGLRenderer {
  constructor(parameters = {}) {
    ...
    // We track this so that the nested render call gets its list and state isolated
    // from the parent render call.
    ...
    // EP: Remove and follow up with usage
    ...
    try {
      ...
      // if ( 'setAttribute' in canvas ) canvas.setAttribute( 'data-engine', `rl r${REVISION}` );
    ...
    this.clear = (color = true, depth = true, stencil = true) => {
      let bits = 0;
    ...
    this.render = function (scene, camera) {
      ...
      if (_currentRenderTarget !== null) {
        // resolve multisample render-buffers to a single-sample texture if necessary

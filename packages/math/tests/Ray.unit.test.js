...
import { eps, one3, posInf3, two3, zero3 } from './math-constants.js';
...
describe('Maths', () => {
  describe('Ray', () => {
    ...
    test('intersectSphere', () => {
      ...
      // sphere (radius of 2) located in front of, but too far right of ray a0,
      ...
      // sphere (radius of 1) located to the left of ray a1, should result in
      // intersection at 0, 1, 1
      b = new Sphere(new Vector3(-1, 1, 1), 1);
      a1.intersectSphere(b, point);
      expect(point.distanceTo(new Vector3(0, 1, 1)) < TOL).toBeTruthy();

      // sphere (radius of 1) located in front of ray a0, should result in
      // intersection at 0, 0, -1
      b = new Sphere(new Vector3(0, 0, -2), 1);
      a0.intersectSphere(b, point);
      expect(point.distanceTo(new Vector3(0, 0, -1)) < TOL).toBeTruthy();

      // sphere (radius of 2) located in front & right of ray a0, should result in
      // intersection at 0, 0, -1, or left-most edge of sphere
      b = new Sphere(new Vector3(2, 0, -1), 2);
      a0.intersectSphere(b, point);
      expect(point.distanceTo(new Vector3(0, 0, -1)) < TOL).toBeTruthy();

      // same situation as above, but move the sphere a fraction more to the right,
      // and ray a0 should now just miss
      b = new Sphere(new Vector3(2.01, 0, -1), 2);
      a0.intersectSphere(b, point.copy(posInf3));
      expect(point.equals(posInf3)).toBeTruthy();

      //
      // test situations where the ray origin is inside the sphere
      //

      // sphere (radius of 1) center located at ray a0 origin / sphere surrounds
      //  the ray origin, so the first intersect point 0, 0, 1, is behind ray a0.
      // Therefore, second exit point on back of sphere will be returned: 0, 0, -1
      // thus keeping the intersection point always in front of the ray.
      b = new Sphere(zero3.clone(), 1);
      a0.intersectSphere(b, point);
      expect(point.distanceTo(new Vector3(0, 0, -1)) < TOL).toBeTruthy();

      // sphere (radius of 4) center located behind ray a0 origin / sphere surrounds
      // the ray origin, so the first intersect point 0, 0, 5,  is behind ray a0.
      // Therefore, second exit point on back of sphere will be returned: 0, 0, -3
      // thus keeping the intersection point always in front of the ray.
      b = new Sphere(new Vector3(0, 0, 1), 4);
      a0.intersectSphere(b, point);
      expect(point.distanceTo(new Vector3(0, 0, -3)) < TOL).toBeTruthy();

      // sphere (radius of 4) center located in front of ray a0 origin / sphere surrounds
      // the ray origin, so the first intersect point 0, 0, 3, is behind ray a0.
      // Therefore, second exit point on back of sphere will be returned: 0, 0, -5
      // thus keeping the intersection point always in front of the ray.
      b = new Sphere(new Vector3(0, 0, -1), 4);
      a0.intersectSphere(b, point);

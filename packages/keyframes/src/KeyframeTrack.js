...
class KeyframeTrack {
  constructor(name, times, values, interpolation) {
    ...
    this.times = KeyframeUtils.convertArray(times, this.TimeBufferType);
    this.values = KeyframeUtils.convertArray(values, this.ValueBufferType);
  ...
  validate() {
    ...
    for (let i = 0; i !== nKeys; i++) {
      ...
      if (typeof currTime === 'number' && isNaN(currTime)) {
        console.warn('KeyframeTrack: Time is not a valid number.', this, i, currTime);
        valid = false;
        break;
      }

      if (prevTime !== null && prevTime > currTime) {
        console.warn('KeyframeTrack: Out of order keys.', this, i, currTime, prevTime);
        valid = false;
        break;
      }
    ...
    if (values !== undefined) {
      if (KeyframeUtils.isTypedArray(values)) {
        for (let i = 0, n = values.length; i !== n; ++i) {
          const value = values[i];

          if (isNaN(value)) {
            console.warn('KeyframeTrack: Value is not a valid number.', this, i, value);
            valid = false;
            break;
          }
        }

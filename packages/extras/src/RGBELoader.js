import { toHalfFloat } from '@renderlayer/buffers';
import { DataTextureLoader } from '@renderlayer/loaders';
import { FloatType, HalfFloatType, LinearFilter, LinearSRGBColorSpace } from '@renderlayer/shared';
...
class RGBELoader extends DataTextureLoader {
  ...
  parse(buffer) {
    const /* default error routine.  change this to change error handling */ rgbe_read_error = 1;

    const rgbe_write_error = 2;
    const rgbe_format_error = 3;
    const rgbe_memory_error = 4;

    const rgbe_error = (rgbe_error_code, msg) => {
    ...
    const /* offsets to red, green, and blue components in a data (float) pixel */
      //RGBE_DATA_RED = 0,
      //RGBE_DATA_GREEN = 1,
      //RGBE_DATA_BLUE = 2,
    ...
    const fgets = (buffer, lineLimit, consume) => {
      const chunkSize = 128;
    ...
    switch (this.type) {
      case FloatType:
        ...
        floatArray = new Float32Array(numElements * 4);
      ...
      case HalfFloatType:
        numElements = image_rgba_data.length / 4;
        halfArray = new Uint16Array(numElements * 4);

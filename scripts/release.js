...
const isCorePackage = (pkgName) => {
  ...
  if (pkgName === 'renderlayer') {
    return true;
  }

  return (
    pkgName.startsWith('@renderlayer') && packages.includes(pkgName.replace(/^@renderlayer\//, ''))
  );
};
...
async function main() {
  ...
  //   try {
  //     const pkgName = renamePackageToCanary('renderlayer');
...
async function getCIResult() {
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/epreston/renderlayer/actions/runs?head_sha=${sha}` +
...
async function isInSyncWithRemote() {
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/epreston/renderlayer/commits/${branch}?per_page=1`
    );
    const data = await res.json();
    if (data.sha === sha) {
      return true;
